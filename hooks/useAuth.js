...
export const useAuth = () => {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      const tokenString = await setSession(accessToken, refreshToken, user);
  ...
  const logout = useCallback(async () => {
    ...
    // used again. Local logout still succeeds even if this fails (e.g.
    // no network, token already expired).
    try {
      ...
      console.warn(
        "Server-side logout failed (logging out locally anyway):",
        error.message,
      );
    }
